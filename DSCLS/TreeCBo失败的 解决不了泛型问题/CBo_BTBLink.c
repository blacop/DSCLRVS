...
//这里二叉树 节点类型一个文件变换了三次，所以报错：重定义:不同的基类型
//不用泛型解决不了报错了
...
#define Nil 0 //具体应该调用 BiTreeEmpty(BiTree T) 函数。这里做个简化,原版代码没有这句，是伪代码

/* 二叉树的二叉链表存储(存储结构由c6-2.h定义)的基本操作(22个) */
...
//引入单链队列
#ifndef CBo_LQueue_c //单链队列 存储结构 操作方法 实现
#define CBo_LQueue_c
#include "CBo_LQueue.c" //引用此文件
#endif //!CBo_LQueue_c
